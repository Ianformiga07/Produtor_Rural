...
IF REQUEST("Operacao") = 1 THEN 'Cadastrar
	...
	response.Redirect("produtor.asp?Operacao=2&CpfVisualizar="&replace(replace(request.form("txtcpf"),".",""),"-","")&"")
	'end if
...
ELSEIF REQUEST("Operacao") = 3 THEN 'ALTERAR
	...
	Session("CPF_Usu") = replace(replace(request.form("txtcpf"),".",""),"-","")
	call fechaConexao
	Response.Redirect("produtor.asp")
	'end if
END IF
%>
...
function cadastrar()
{
	...
	document.frmCadastro.action = "produtor.asp";
...
function novo()
{
	window.location.href = "produtor.asp";
}
</script>
</head>
<body>
